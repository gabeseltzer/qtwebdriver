...
#
{
  'rules': [         
  {
    ...
    'action': [ 'python3', 'do_if_modified.py', '<(QT_LIBEXEC_PATH)/uic', '<(RULE_INPUT_PATH)', '<(INTERMEDIATE_DIR)/ui_<(RULE_INPUT_ROOT).h' ],
  ...
  },
  {
    ...
    'action': [ 'python3', 'do_if_modified.py','<(QT_LIBEXEC_PATH)/moc', '<(RULE_INPUT_PATH)', '<(INTERMEDIATE_DIR)/moc_<(RULE_INPUT_ROOT).cc' ],
  ...
  },
  {
    ...
    'action': [ '<(QT_LIBEXEC_PATH)/rcc', '<(RULE_INPUT_PATH)', '-name', '<(RULE_INPUT_ROOT)', '-o', '<(INTERMEDIATE_DIR)/rcc_<(RULE_INPUT_ROOT).cc' ],
    'message': 'Generating <(RULE_INPUT_ROOT).cc.',
  } ],
}
